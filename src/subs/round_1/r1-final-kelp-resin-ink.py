...
class InkStrategy(Strategy):
    def __init__(
        ...
        """
        - window_size: number of recent mid-prices used to compute the moving average.
        - threshold: minimum absolute difference between current price and moving average to trigger a trade.
        - trade_unit: fixed quantity per trade signal.
        """
        super().__init__(symbol, limit)
    ...
    def act(self, state: TradingState) -> None:
        # Ensure we have market data for our symbol.
        ...
        # Compute the current mid price using the best bid and ask.
        ...
        # Get current position; positive indicates a long position.
        position = state.position.get(self.symbol, 0)

        # Signal: If price is significantly higher than the moving average, sell expecting a decline.
        if deviation > self.threshold and position > -self.limit:
            # Calculate how many units we can sell without breaching our position limit.
            quantity = min(self.trade_unit, position + self.limit)
            if quantity > 0:
                self.sell(mid_price, quantity)

        # Signal: If price is significantly lower than the moving average, buy expecting a rise.
