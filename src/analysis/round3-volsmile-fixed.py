...
class BlackScholesStrategy(SignalStrategy):
    ...
    def act(self, state: TradingState) -> None:
        if (
            ...
            or len(state.order_depths[self.underlying_symbol].buy_orders) == 0
            or len(state.order_depths[self.underlying_symbol].sell_orders) == 0
        ):
            return
        if (
            self.symbol not in state.order_depths
            or len(state.order_depths[self.symbol].buy_orders) == 0
            or len(state.order_depths[self.symbol].sell_orders) == 0
        ):
            return
...
class PicnicBasket1Strategy(BasketStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        basket_weights = {"CROISSANTS": 6, "JAMS": 3, "DJEMBES": 1}
...
class PicnicBasket2Strategy(BasketStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        basket_weights = {"CROISSANTS": 4, "JAMS": 2}
...
# NEW VOUCHER STRATEGY BASED ON THE VOLATILITY SMILE
# This class requires pre-calculated volatility smile parameters (a, b, c) to be provided.
class VolcanicVoucherSmileStrategy(Strategy):
    def __init__(
        self,
        symbol: Symbol,
        limit: int,
        strike_price: int,
        option_type: str,
        vol_smile: tuple[float, float, float],
        threshold: float = 0.5,
        trade_unit: int = 80,
    ) -> None:
        super().__init__(symbol, limit)
        ...
        self.vol_smile = vol_smile  # Pre-calculated (a, b, c)
        self.threshold = threshold
        self.trade_unit = trade_unit
        self.initial_time_to_expiry = (365 - 0) / 365
    ...
    def get_TTE(self, state: TradingState) -> float:
        ...
        frac = state.timestamp / mill  # Fraction of the day elapsed.
    ...
    def act(self, state: TradingState) -> None:
        # Ensure underlying order depth is available.
        if "VOLCANIC_ROCK" not in state.order_depths:
            return
        underlying_od = state.order_depths["VOLCANIC_ROCK"]
        underlying_mid = self.get_mid_price(underlying_od)
        if underlying_mid is None:
            return
        # Get voucher order depth.
        if self.symbol not in state.order_depths:
            return
        voucher_od = state.order_depths[self.symbol]
        voucher_mid = self.get_mid_price(voucher_od)
        if voucher_mid is None:
            return
        T = self.get_TTE(state)
        try:
            m = math.log(self.strike_price / underlying_mid) / math.sqrt(T)
        except Exception:
            return
        computed_iv = self.compute_implied_vol(
            underlying_mid, self.strike_price, T, voucher_mid
        )
        a, b, c = self.vol_smile
        fitted_iv = a * m**2 + b * m + c
        diff = computed_iv - fitted_iv
        if diff > self.threshold:
            if voucher_od.buy_orders:
                price = max(voucher_od.buy_orders.keys())
                self.sell(price, self.trade_unit)
        elif diff < -self.threshold:
            if voucher_od.sell_orders:
                price = min(voucher_od.sell_orders.keys())
                self.buy(price, self.trade_unit)
...
# NEW VOUCHER STRATEGIES USING THE VOLATILITY SMILE BASE CLASS
class VolcanicVoucher9500Strategy(VolcanicVoucherSmileStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        # Supply your pre-calculated volatility smile coefficients for this voucher
        precomputed_smile = (1.4285, -0.00013, 0.02015)
        super().__init__(
            symbol,
            limit,
            strike_price=9500,
            option_type="call",
            vol_smile=precomputed_smile,
            threshold=0.04,
        )


class VolcanicVoucher9750Strategy(VolcanicVoucherSmileStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        precomputed_smile = (1.4285, -0.00013, 0.02015)
        super().__init__(
            symbol,
...
class VolcanicVoucher10000Strategy(VolcanicVoucherSmileStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        precomputed_smile = (1.4285, -0.00013, 0.02015)
        super().__init__(
            symbol,
...
class VolcanicVoucher10250Strategy(VolcanicVoucherSmileStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        precomputed_smile = (1.4285, -0.00013, 0.02015)
        super().__init__(
            symbol,
...
class VolcanicVoucher10500Strategy(VolcanicVoucherSmileStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        precomputed_smile = (1.4285, -0.00013, 0.02015)
        super().__init__(
            symbol,
    ...
    def __init__(self) -> None:
        ...
        self.strategies: dict[Symbol, Strategy] = {
            ...
            for symbol, clazz in {
                ...
                # Replace voucher strategies with the new ones using a pre-calculated vol smile.
